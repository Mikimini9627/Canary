...
namespace Canary
{
    public partial class MainForm : Form
    {
        ...
        private void UpdateTaisyoKensu(object? sender, EventArgs e)
        {
            ...
            {
                // 対象件数を取得
                var tasiyoKensu = SearchTaisyoKensu();

                Invoke(() =>
                {
                    // 対象件数を取得
                    edtKensu.Text = tasiyoKensu.ToString() + "件";
                });
            });
        }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < chunks.Count; i++)
            {
                ...
                task_list.Add(Task.Run(() =>
                {
                    ...
                    {
                        Debug.WriteLine("スレッド番号 = " + (a + 1).ToString() + " : オフセット = " + offset_list.ToArray()[j].ToString());
                        ...
                        {
                            Debug.WriteLine("スレッド番号 = " + (a + 1).ToString() + " : ループをBREAK");
                            break;
                        }
            ...
            // エクセルに落とし込む
            SaveChintaiData(result_estates, result_rooms);
        }
        ...
        private void SaveChintaiData(List<JsonNode> estates_data, List<JsonNode> rooms_data)
        {
            // 保存する
            ...
            // ヘッダー生成
            var header = new[] { "No.", "建物名", "住所", "建築年", "総階数(地下含む)", "建物種類", "構造", "部屋番号", "階数", 
                "敷金", "礼金", "家賃(共益費込)", "入居可能タイミング", "レイアウト", "更新日時", "間取り", "間取り画像"};
            ...
            for (int i = 0; i < rooms_data.Count; i++)
            {
                ...
                // ハイパーリンクのみ挿入
                ws.Cell(i + 2, row.Count + 1).SetHyperlink(new XLHyperlink(rooms_data[i]["layoutImageUrl"]["value"].ToString()));
                ws.Cell(i + 2, row.Count + 1).Value = rooms_data[i]["layoutImageUrl"]["value"].ToString();
            }
